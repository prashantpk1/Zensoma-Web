...
class CouponController extends Controller
{
    ...
    public function store(CouponRequest $request)
    {
        ...
        try {
            ...
            $data->save();
            if (!empty($data)) {
                return response()->json(['status' => '1', 'success' => 'Coupon added successfully.']);
            }
        } catch (Exception $ex) {
    ...
    public function update(CouponRequest $request, string $id)
    {
        ...
        try {
            ...
            $data->save();
            if (!empty($data)) {
            return response()->json(['status' => '1', 'success' => 'Coupon edit Successfully']);
            }
        } catch (Exception $ex) {
            return response()->json(
                ['success' => false, 'message' => $ex->getMessage()]
    ...
    public function destroy(string $id)
    {
        ...
        try {
            ...
            return response()->json(['status' => '1', 'success' => 'Coupon deleted successfully']);
        } catch (\Exception $e) {
    ...
    public function couponStatus($id)
    {
        try {
            ...
            $data->update();
            DB::commit(); // Commit Transaction
            return response()->json(['status' => '1', 'success' => 'Coupon ' . $message . ' Successfully']);
        } catch (\Exception $e) {
            DB::rollBack(); //Rollback Transaction
            return redirect()->back()->withErrors(['error' => $e->getMessage()]);
        } catch (\Throwable $e) {

...
class ContentManagementController extends Controller
{
    ...
    public function index(Request $request)
    {
        //
        if ($request->ajax()) {
            $data = ContentManagement::where('is_delete', 0)->with('category')->orderBY('id', 'DESC')->get();
            return Datatables::of($data)->addIndexColumn()
                ->addColumn('action', function ($row) {
                    $alert_delete = "return confirm('Are you sure want to delete !')";
                    $btn = "<ul class='action'>";
                    if ($row->status == 1) {
                        $btn = $btn . '<li class="delete"> <a   href="javascript:void(0)" href="' . route('content.status', $row->id) . '" title="Deactivate" class="status-change" data-url="' . route('content.status', $row->id) . '"><i class="fa fa-close"></i></a>  </li> ';
                    } else {
                        $btn = $btn . ' <li class="edit"> <a   href="javascript:void(0)" href="' . route('content.status', $row->id) . '"   class="status-change" title="Activate" data-url="' . route('content.status', $row->id) . '"><i class="icon-check"></i></a></li> ';
                    }
                    $btn = $btn . '<li class="edit"> <a  href="' . route('content.edit', $row->id) . '" title="Edit"><i class="icon-pencil-alt"></i></a></li>';

                    $btn = $btn . ' <li class="delete"><a href="" data-url="' . route('content.destroy', $row->id) . '" class="destroy-data" title="Delete"> <i class="icon-trash"></i></a></li> </ul>';
                    return $btn;
                })
    ...
    public function edit(string $id)
    {
        ...
        try {
            ...
            if (!empty($data)) {
                return view('Admin.Content.edit', compact('data','count'));
            }
        } catch (Exception $ex) {
            return response()->json(
                ['success' => false, 'message' => $ex->getMessage()]
    ...
    public function update(Request $request, string $id)
    {
        ...
        try {
            ...
            if (!empty($data)) {
                return response()->json(['status' => '1', 'success' => 'Content edit Successfully']);
            }
        } catch (Exception $ex) {
            return response()->json(
                ['success' => false, 'message' => $ex->getMessage()]
    ...
    public function destroy(string $id)
    {
        ...
        try {
            ...
            return response()->json(['status' => '1', 'success' => 'Content deleted successfully']);
        } catch (\Exception $e) {
    ...
    public function contentStatus(string $id)
    {
        ...
        try {
            ...
            $data->update();
            DB::commit(); // Commit Transaction
            return response()->json(['status' => '1', 'success' => 'Content ' . $message . ' Successfully']);
        } catch (\Exception $e) {
            DB::rollBack(); //Rollback Transaction
            return redirect()->back()->withErrors(['error' => $e->getMessage()]);
        } catch (\Throwable $e) {

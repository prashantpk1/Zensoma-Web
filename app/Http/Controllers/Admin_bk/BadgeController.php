...
class BadgeController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $data->save();
            if (!empty($data)) {
                return response()->json(['status' => '1', 'success' => 'Bagde added successfully.']);
            }
        } catch (Exception $ex) {
    ...
    public function destroy(string $id)
    {
        ...
        try {
            ...
            return response()->json(['status' => '1', 'success' => 'Badge deleted successfully']);
        } catch (\Exception $e) {
    ...
    public function badgeStatus($id)
    {
        try {
            ...
            $data->update();
            DB::commit(); // Commit Transaction
            return response()->json(['status' => '1', 'success' => 'Badge ' . $message . ' Successfully']);
        } catch (\Exception $e) {
            DB::rollBack(); //Rollback Transaction
            return redirect()->back()->withErrors(['error' => $e->getMessage()]);
        } catch (\Throwable $e) {

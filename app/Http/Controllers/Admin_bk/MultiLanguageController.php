...
class MultiLanguageController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $data->save();
            if (!empty($data)) {
                return response()->json(['status' => '1', 'success' => 'Key added successfully.']);
            }
        } catch (Exception $ex) {
    ...
    public function destroy(string $id)
    {
        ...
        try {
            ...
            return response()->json(['status' => '1', 'success' => 'Key deleted successfully']);
        } catch (\Exception $e) {
    ...
    public function multilanguageStatus($id)
    {
        try {
            ...
            $data->update();
            DB::commit(); // Commit Transaction
            return response()->json(['status' => '1', 'success' => 'Key ' . $message . ' Successfully']);
        } catch (\Exception $e) {
            DB::rollBack(); //Rollback Transaction
            return redirect()->back()->withErrors(['error' => $e->getMessage()]);
        } catch (\Throwable $e) {

...
class  TagController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $data->save();
            if (!empty($data)) {
                return response()->json(['status' => '1', 'success' => 'Tag added successfully.']);
            }
        } catch (Exception $ex) {
    ...
    public function update(Request $request, string $id)
    {
        ...
        try {
            ...
            $data->update();
            if (!empty($data)) {
                return response()->json(['status' => '1', 'success' => 'Tag edit Successfully']);
            }
        } catch (Exception $ex) {
            return response()->json(
                ['success' => false, 'message' => $ex->getMessage()]
    ...
    public function destroy(string $id)
    {
        //
          ...
          try {
            ...
            {
                return response()->json(['status' => '1', 'success' => 'Default Tag can not detele']);
            }
            $data->is_delete = 1;
            $data->update();
            DB::commit(); // Commit Transaction
            return response()->json(['status' => '1', 'success' => 'Tag deleted successfully']);
        } catch (\Exception $e) {
    ...
    public function TagStatus(string $id)
    {
        ...
        try {
            ...
            $data->update();
            DB::commit(); // Commit Transaction
            return response()->json(['status' => '1', 'success' => 'Tag ' . $message . ' Successfully']);
        } catch (\Exception $e) {
            DB::rollBack(); //Rollback Transaction
            return redirect()->back()->withErrors(['error' => $e->getMessage()]);
        } catch (\Throwable $e) {

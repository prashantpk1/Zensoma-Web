...
class CmsController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $data->save();
            if (!empty($data)) {
                return response()->json(['status' => '1', 'success' => 'Cms added successfully.']);
            }
        } catch (Exception $ex) {
    ...
    public function destroy(string $id)
    {
        ...
        try {
            ...
            return response()->json(['status' => '1', 'success' => 'Cms deleted successfully']);
        } catch (\Exception $e) {
    ...
    public function cmsStatus($id)
    {
        try {
            ...
            $data->update();
            DB::commit(); // Commit Transaction
            return redirect()->back()->with('success', 'Cms ' . $message . ' Successfully');
        } catch (\Exception $e) {
            DB::rollBack(); //Rollback Transaction
            return redirect()->back()->withErrors(['error' => $e->getMessage()]);
        } catch (\Throwable $e) {

...
use App\Models\ChallengeFriendDetail;
...
class ChallengeFriendController extends Controller
{
                   ...
                   if(empty($check_challenge))
                    {
                        ...
                        foreach($session->session_videos as $session_video)
                        {
                              $data_detail = new ChallengeFriendDetail();
                              $data_detail->challenge_friend_id = $data->id;
                              $data_detail->session_video_id = $session_video->id;
    ...
    public function complateChallenge(Request $request)
    {
        ...
        try{
            $data_challenge = ChallengeFriendDetail::where('challenge_friend_id',$request->challenge_id)->where('session_video_id',$request->session_video_id)->where('status',0)->first();
            if($data_challenge)
            {
                        $data_challenge->status = "1";
                        $data_challenge->update();

                        $data_challenge = ChallengeFriendDetail::where('challenge_friend_id',$request->challenge_id)->where('status',0)->first();

                        if(empty($data_challenge))
                        {
                                ...
                                $data_challenge_friend = ChallengeFriend::find($request->challenge_id);
                                if($data_challenge_friend)
                                {
                                    $data_challenge_friend->status = 1;
                                    $data_challenge_friend->update();
                                }
                        }
